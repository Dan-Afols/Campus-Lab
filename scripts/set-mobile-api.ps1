...
if (-not $resolvedBase) {
  $resolvedBase = "http://10.0.2.2:4000/api/v1"
}

Set-Or-AddEnvValue -Path $mobileEnvPath -Key "EXPO_PUBLIC_API_BASE_URL" -Value $resolvedBase
Write-Host "Updated apps/mobile/.env" -ForegroundColor Green
Write-Host "EXPO_PUBLIC_API_BASE_URL=$resolvedBase"
